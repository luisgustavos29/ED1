...
#include "lladae2.h"
    ...
    do {
        ...
        for (int i = 0; i + gap < Tamanho_lista(Pt1); i++) {
            ...
            for (int j = 0; j < gap; j++) {
                if (proximo == NULL) break; // Verifica se proximo é NULL
                proximo = proximo->prox;
            }

            if (proximo != NULL && atual->info > proximo->info) {
                // Troca os nós atual e proximo
